 ...
 const boxIds=`uqyoeiznvmzrstpkgnsacjxwld
...
let arrBoxIds = boxIds.split("\n");

let counterTwice = 0;
let counterThrice = 0;


arrBoxIds.forEach(function(element){
...
console.log("checksum:" + counterTwice * counterThrice); //5681

//Part Two 

loop:
for (var i = 0; i < arrBoxIds.length; i++) {   //iterates through strings one by one

 for (var j = i+1; j < arrBoxIds.length; j++) {   //compares the string to all the strings below

   let tempString = []; let k = 0; let countDiff = 0;

   while( k < 26 ){

     if (arrBoxIds[i][k] === arrBoxIds[j][k]) {
         tempString.push(arrBoxIds[i][k]);
      }
      else{
       countDiff++;
      }
      ...
      if (tempString.length === 25) {
       ...
       console.log("Common characters:" + tempString.join("")); //uqyoeizfvmbistpkgnocjtwld
       break loop;
      }
   }
 }
